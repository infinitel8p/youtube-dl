...
pnpm --dir web run build                   # produces web/dist/
...
Use `--onedir` on **macOS**: a `--onefile` `.app` is a bootloader that extracts to a temp
dir and spawns a second process, which on macOS shows a double Dock icon and only launches
on the second click. Linux can use either (`--onefile` gives a single binary, as the release
workflow does).

```bash
pyinstaller --onedir --noconfirm --windowed \
